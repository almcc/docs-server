...
class Parser(object):
    def __init__(self, base_dir, jarvis_api):
    ...
    def parse_documentation_directory(self):
        ...
        for product in products:
            ...
            if os.path.isdir(product_path):
                logger.debug('Found {}/ product.'.format(product_path))
                ...
                logger.error('Found file {} in product directory, it should not be here.'
                             .format(product_path))
    ...
    def parse_product_directory(self, directory, product_id):
        ...
        for release in releases:
            ...
            if os.path.isdir(release_path):
                logger.debug('Found {}/ release.'.format(release_path))
                ...
                logger.error('Found file {} in release directory, it should not be here.'
                             .format(release_path))
    ...
    def parse_release_directory(self, directory, release_id):
        ...
        for artifact in artifacts:
            ...
            if os.path.isdir(artifact_path):
                logger.debug('Found {}/ artifact.'.format(artifact_path))
                self.get_or_create_artifact(name=artifact, release_id=release_id)
            else:
                logger.error('Found file {} in artifact directory, it should not be here.'
                             .format(artifact_path))

    def get_or_create_product(self, name):
        product_id = self.get_product_id(name)
        if product_id is None:
            logger.debug('Product {} does not exist.'.format(name))
            return self.create_product(name=name)
        else:
            logger.debug('Product {} already exists.'.format(name))
            return product_id
    ...
    def create_product(self, name):
        ...
        response = requests.post('{}/products'.format(self.jarvis_api), json=new)
        if response.status_code == 201:
            logger.debug('Product {} created.'.format(name))
    ...
    def get_or_create_release(self, name, product_id):
        ...
        if release_id is None:
            logger.debug('Release {} does not exist.'.format(name))
            return self.create_release(name=name, product_id=product_id)
        else:
            logger.debug('Release {} already exists.'.format(name))
            return release_id
    ...
    def create_release(self, name, product_id):
        ...
        response = requests.post('{}/releases'.format(self.jarvis_api), json=new)
        if response.status_code == 201:
            logger.debug('Release {} created.'.format(name))
    ...
    def get_or_create_artifact(self, name, release_id):
        ...
        if artifact_id is None:
            logger.debug('Artifact {} does not exist.'.format(name))
            return self.create_artifact(name=name, release_id=release_id)
        else:
            logger.debug('Artifact {} already exists.'.format(name))
            return artifact_id
    ...
    def create_artifact(self, name, release_id):
        ...
        response = requests.post('{}/artifacts'.format(self.jarvis_api), json=new)
        if response.status_code == 201:
            logger.debug('Artifact {} created.'.format(name))
    ...
    def get_remote_paths(self):
        response = requests.get('{}/path-tree'.format(self.jarvis_api))
    ...
    def remove_paths(self, paths):
        for path in paths:
            parts = path.split('/')
            if len(parts) == 3:
                logger.debug('Going to remove artifact {}/{}/{}'.format(*parts))
                self.delete_artifact(product=parts[0], release=parts[1], artifact=parts[2])
            if len(parts) == 2:
                logger.debug('Going to remove release {}/{}'.format(*parts))
                self.delete_release(product=parts[0], release=parts[1])
            if len(parts) == 1:
                logger.debug('Going to remove product {}'.format(*parts))
                self.delete_product(product=parts[0])

    def delete_product(self, product):
        product_id = self.get_product_id(product)
        if product_id:
            response = requests.delete('{}/products/{}'.format(self.jarvis_api, product_id))
    ...
    def delete_release(self, product, release):
        ...
        if product_id:
            ...
            if release_id:
                response = requests.delete('{}/releases/{}'.format(self.jarvis_api, release_id))
                if response.status_code == 204:
                    logger.info('Deleted release {}'.format(release))
            ...
            if release_id:
                ...
                if artifact_id:
                    response = requests.delete('{}/artifacts/{}'
                                               .format(self.jarvis_api, artifact_id))
                    if response.status_code == 204:
                        logger.info('Deleted artifact {}'.format(artifact))

    def get_product_id(self, product_name):
        product_id = None
        response = requests.get('{}/products?name={}'.format(self.jarvis_api, product_name))
        if response.status_code == 200:
    ...
    def get_release_id(self, release_name, product_id):
        ...
        response = requests.get('{}/releases?name={}&product={}'
                                .format(self.jarvis_api, release_name, product_id))
        if response.status_code == 200:
    ...
    def get_artifact_id(self, artifact_name, release_id):
        ...
        response = requests.get('{}/artifacts?name={}&release={}'
                                .format(self.jarvis_api, artifact_name, release_id))
        if response.status_code == 200:
...
if __name__ == '__main__':
    ...
    base_dir = '/opt/docs'
    jarvis_api = 'http://localhost:80/api/v1'
    parser = Parser(base_dir=base_dir, jarvis_api=jarvis_api)
    parser.parse()
